...
class OrderLogic
{
    ...
    {
        if(!isset($data['id'] )||!isset($data['count'] )||!isset($data['price_map'] )||!isset($data['guest_info'])) error(40000,'参数不全');
    ...
    {
        if(($getOrder['status']==3||$getOrder['status']==6)&&($getOrder['refund_status']==0||$getOrder['refund_status']==2)) $refund = true;
        else                      $refund = false;
        $list = [
    ...
    {
        if($order['status']  != '3'||$order['refund_status']==1||$order['refund_status']==3) error(40000,'该订单无法进行退款操作！');
        return true;
    }
