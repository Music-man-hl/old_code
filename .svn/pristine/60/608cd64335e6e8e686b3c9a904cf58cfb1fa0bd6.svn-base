...
class ShopLogic
{
    ...
    public function lists($all_param){
        ...
        if(empty($getGroup))error(40000,'获取店铺信息错误');
        $getShop = $this->handle->getShopLists($channel,$all_param,$total);
        if(empty($getShop)) success(['list'=>[],'shop_type'=>'','total_count'=>'']);
        ...
        success(['list'=>$list,'shop_type'=>$getGroup['group'],'total_count'=>$total]);
    ...
    public function index($all_param)
    {
        ...
        if(empty($all_param['sub_shop_id']))
        {
            ...
            if($subShopId === false) error(40000,'首页门店错误！');
        }
        else
        {
        ...
        {
            if($v['type']==2&&!empty($v['citycode'])) continue;
            $district .= $v['name'];
        }
        $location  = array(
    ...
    public function desc($all_param)
    {
        ...
        {
            $subShopId = Room::validSubId($shopId);
            if($subShopId === false) error(40000,'详情门店错误！');
        }
        else
        {
            $subShopId= encrypt($all_param['sub_shop_id'],4,false);//门店id
    ...
    public function facilities($all_param)
    {
        ...
        {
            $subShopId = Room::validSubId($shopId);
            if($subShopId === false) error(40000,'设施门店错误！');
        }
        else
        {
            $subShopId= encrypt($all_param['sub_shop_id'],4,false);//门店id
