...
class OrderLogic
{
    ...
    {
        if(empty($data['voucher_id'] )||!isset($data['count'])) error(40000,'参数不全');
        ...
        else                   error(40000,'未找到产品！');
        if(($product['allot']-$total_order-$product['sales']-$count)<0) error(40000,'库存不足！');
        if($count<$product['min']||$count>$product['max']) error(40000,'购买数量有误！');

        $level      = OrderModel::getStandard($product['level1'],$product['level2']);
        if(empty($level))  error(40000,'未找到层级！');
        $levelArr       = [];
        foreach ($level as $v)
        {
            $levelArr[$v['level']] = $v;
        }
        if(isset($levelArr['2'])) $name = $levelArr['2']['value'];
        else                      $name = $levelArr['1']['value'];
        $shop_arr   = OrderModel::getShop($shop_id);
        if(empty($shop_arr))  error(50000,'没有找到店铺');
        $shop = $shop_arr[0];

        if(bcmul($product['sale_price'],$count,2) != $total_price) error(40000,'价格不正确');
        //校验库存和价格是否一致
    ...
    static function orderDetail($getOrder,$data)
    {

        $order_voucher    =   OrderModel::getOrderVoucherByOrder( $getOrder['order']);
        $product          =   OrderModel::getProductByIdForOrder( $data['product_id']);
        if(!isset($order_voucher[0])) error(40000,'券不存在！');

        $item   =   [];
        if($data['booking_end']<(NOW-86400)||$getOrder['status'] == Status::ORDER_UNPAY||($getOrder['status'] == Status::ORDER_CLOSE&&in_array($getOrder['refund_status'],[0,3]))||($getOrder['status'] != Status::ORDER_BOOKING_FAIL&&in_array($getOrder['refund_status'],[1]))) $status = 99;
        if($getOrder['status'] == Status::ORDER_BOOKING_FAIL&&in_array($getOrder['refund_status'],array(1))) $status = 98;
        foreach ($order_voucher as $vv)
        {
            if(in_array($vv['status'],array(Status::TICKET_BOOKING,Status::TICKET_CONFIRM,Status::TICKET_COMPLETE))) $refund_status = false;
            if($getOrder['status'] == Status::ORDER_CLOSE&&$vv['status']==Status::TICKET_BOOKING_FAIL) $status = 98;
            ...
            {
                if(isset($status)) $statusEx   =   $status;
            }
            if($getOrder['refund_status'] == 3&&$vv['status']==0) $status = 99;
            else
            {
                if(isset($status)) $statusEx   =   $status;
            }
            ...
            if(isset($statusEx))$status = $statusEx;
        }
        $voucherArr[]     =   [
        ...
        if((($getOrder['status']==3||$getOrder['status']==6)&&($getOrder['refund_status']==0||$getOrder['refund_status']==2))&&$data['booking_end']>(NOW-86400))      $refund = true;
        else                      $refund = false;

        if(isset($refund_status)) $refund = $refund_status;
        $list = [
            "product_name"  => $data['product_name'],
            "product_id"    => encrypt($getOrder['product'],1),
            "product_cover" => picture($data['bucket'],$data['cover']),
            "order_count"   => $data['count'], // 订单件数
            "order_price"   => floatval($data['price_total']/$data['count']), // 该订单预定期间的均价
    ...
    static function payCreateVild($orders)
    {
    ...
    //订单预约
    static function booking($data)
    {
        if(!isset($data['adult'])) error(40000,'预约人信息不存在！');
        $voucherArr     =   OrderModel::getVoucherByOrderId($data['order_id'],$data['id'],$data['user']);
        if(!isset($voucherArr[0])) error(40000,'预约券信息不存在！');
        $voucher        =   $voucherArr[0];
        if(count($voucherArr)>1) $sub_status =   1;
        else                    $sub_status =   0;
        ...
        {
            if($v['voucher_status'] != Status::TICKET_BOOKING) $status = 1;
        }
        if(empty($product)) error(40000,'产品不存在！');
        $vaild_status   =   [Status::ORDER_PAY,Status::ORDER_BOOKING,Status::ORDER_CONFIRM,Status::ORDER_BOOKING_FAIL];
        if(!in_array($voucher['status'],$vaild_status)) error(40000,'订单状态不允许预约！');
        if($voucher['voucher_status']!=Status::TICKET_DEFAUT&&$voucher['voucher_status']!=Status::TICKET_BOOKING_FAIL) error(40000,'券状态不允许预约！');
        if($jsonData['booking_end']<(NOW-86400)) error(40000,'券预约已过期！');

        $itemDate   =  OrderModel::getVoucherDate($voucherArr[0]['item_id'],$data['checkin']);
        if(empty($itemDate)) error(40000,'当天不可预约！');
        if(($itemDate['allot']-$itemDate['used'])<=0) error(40000,'库存不足无法预约！');
        $people     =   [];
        $user_info  =   [];
        if(isset($data['adult']))
        {
    ...
    static function getProductId($id){
        ...
        if($product['is_coupons'] == '0') error(40000,'该产品不支持优惠券！');
        return $data['pid'];
    }
}